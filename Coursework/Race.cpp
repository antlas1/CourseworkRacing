...
bool Race::canAddTransport(Transport* transport) const {
    if (groundRace && !airRace && !transport->isGround()) return false;
    if (airRace && !groundRace && !transport->isAir()) return false;
